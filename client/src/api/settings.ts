...
import type { NovelGraphPreciseRuntimeReadiness } from "@ai-novel/shared/types/novelGraph";
import type {
  ModelRouteConfig,
...
import { normalizeNovelGraphPreciseRuntimeReadiness } from "@/lib/novelGraphRuntime";
...
export interface GraphEngineRuntimeSettingsStatus {
  enabled: boolean;
  autoAnalyze: boolean;
  engineRoot: string;
  projectsRoot: string;
...
export interface GraphEngineRuntimeImportResult {
  settings: GraphEngineRuntimeSettingsStatus;
...
export async function getGraphEngineRuntimeSettings() {
  const { data } = await apiClient.get<ApiResponse<GraphEngineRuntimeSettingsStatus>>("/settings/graph-engine-runtime");
  return data;
}

export async function getGraphEngineRuntimeReadiness() {
  const { data } = await apiClient.get<ApiResponse<NovelGraphPreciseRuntimeReadiness>>(
    "/settings/graph-engine-runtime/readiness",
  );
  if (data.data) {
...
export async function saveGraphEngineRuntimeSettings(payload: {
  enabled: boolean;
  autoAnalyze: boolean;
  engineRoot: string;
  projectsRoot: string;
  ...
  const { data } = await apiClient.put<ApiResponse<GraphEngineRuntimeSettingsStatus>>(
    "/settings/graph-engine-runtime",
    payload,
  );
  return data;
}

export async function importGraphEngineRuntimeFromProvider(provider: string) {
  const { data } = await apiClient.post<ApiResponse<GraphEngineRuntimeImportResult>>(
    "/settings/graph-engine-runtime/import-provider",
    { provider },
  );
  return data;
}
