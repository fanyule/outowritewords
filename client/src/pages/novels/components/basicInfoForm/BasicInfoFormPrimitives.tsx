...
export function SectionBlock({
  ...
  return (
    <section className="app-shell-panel-soft space-y-3 rounded-2xl p-4">
    ...
    <button
      ...
      className={`rounded-2xl border p-3 text-left transition ${
        selected
          ...
          : "border-border/70 bg-background/80 hover:border-primary/30 hover:bg-primary/[0.035]"
      }`}
    >
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm font-medium text-foreground">{option.label}</div>
        {option.recommended ? (
          <span className="rounded-full border border-primary/15 bg-primary/[0.08] px-2 py-0.5 text-[10px] font-medium text-primary">
            推荐
          </span>
        ) : null}
      </div>
