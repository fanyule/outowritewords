      ...
      {novelListQuery.isPending ? (
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {Array.from({ length: 4 }).map((_, index) => (
            <Card
              key={`loading-${index}`}
              className="app-shell-panel animate-pulse overflow-hidden"
            >
        ...
        <Card className="app-state-panel-danger overflow-hidden">
          <CardHeader className="space-y-3 border-b border-border/60 p-5 sm:p-6">
            <div className="app-section-kicker">System State</div>
            <CardTitle>加载小说列表失败</CardTitle>
            <CardDescription className="app-reading-copy text-muted-foreground">
              当前无法读取项目列表，可以重试一次。
            </CardDescription>
          </CardHeader>
        ...
        <Card className="app-state-panel overflow-hidden">
          <CardHeader className="space-y-3 border-b border-border/60 p-5 sm:p-6">
            <div className="app-section-kicker">Empty State</div>
          ...
          {novels.map((novel) => {
            ...
            return (
              <Card
                ...
                role="link"
                tabIndex={0}
                className="app-shell-panel group cursor-pointer overflow-hidden transition duration-200 hover:-translate-y-0.5 hover:border-primary/45 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:ring-offset-0"
              ...
              >
                ...
                <CardContent className="space-y-4 pt-5">
                  ...
                  {workflowTask ? (
                    <div
                      className={cn(
                        "rounded-xl border border-border/65 bg-card/68 p-3 transition-colors",
                        isWorkflowRunning
                          ? "border-primary/40 bg-primary/10 shadow-[var(--app-card-shadow)]"
                          : "border-border/60 bg-card/58",
                      )}
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        {workflowBadge ? (
                      ...
                      {workflowDescription ? (
                        <div className="mt-2 app-caption-copy">{workflowDescription}</div>
                      ) : null}
                      {isWorkflowRunning ? (
                    ...
                    <div className="app-state-panel-muted p-3 text-xs text-muted-foreground">
