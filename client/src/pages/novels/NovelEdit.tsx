...
export default function NovelEdit() {
  ...
  const [preciseGraphStatusWatchUntil, setPreciseGraphStatusWatchUntil] = useState(0);
  ...
  const preciseGraphStatusQuery = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return Date.now() < preciseGraphStatusWatchUntil ? 3000 : false;
    },
  });
  const preciseGraphReadinessQuery = useQuery({
  ...
  const requestPreciseGraphAnalysisMutation = useMutation({
    mutationFn: () => requestNovelPreciseGraphAnalysis(id, { trigger: "manual", force: true }),
    onSuccess: async () => {
      setPreciseGraphStatusWatchUntil(Date.now() + 45_000);
      await Promise.all([
      ...
      toast.success("精确图谱分析任务已加入队列。");
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "启动精确图谱分析失败。");
    },
  });
  ...
  const invalidateNovelDetail = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.novels.characterRelations(id) }),
      queryClient.invalidateQueries({ queryKey: queryKeys.novels.characterResources(id) }),
      queryClient.invalidateQueries({ queryKey: ["novels", "chapter-plan", id] }),
      queryClient.invalidateQueries({ queryKey: ["novels", "chapter-audit-reports", id] }),
      queryClient.invalidateQueries({ queryKey: ["novels", "state-snapshots", id] }),
    ]);
  };

  const syncPreciseGraphStatusAfterChapterUpdate = async () => {
    setPreciseGraphStatusWatchUntil(Date.now() + 45_000);
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: queryKeys.novels.preciseGraphStatus(id) }),
      queryClient.invalidateQueries({ queryKey: queryKeys.novels.preciseGraph(id) }),
  ...
  const chapterSSE = useSSE({
    ...
    onDone: async () => {
      await Promise.all([
        ...
        syncPreciseGraphStatusAfterChapterUpdate(),
      ]);
  ...
  const isExportingFullJson = exportNovelMutation.isPending
    && exportVariables?.scope === "full"
    && exportVariables?.format === "json";
  const canRunPreciseGraphAnalysis = Boolean(id)
    && Boolean(preciseGraphReadinessQuery.data?.data?.ready)
    && preciseGraphStatusQuery.data?.data?.status !== "queued"
    && preciseGraphStatusQuery.data?.data?.status !== "running"
    && !requestPreciseGraphAnalysisMutation.isPending;
  ...
  return (
    <NovelEditView
      ...
      graphPanel={{
        status: preciseGraphStatusQuery.data?.data ?? null,
        readiness: preciseGraphReadinessQuery.data?.data ?? null,
        isLoading: preciseGraphStatusQuery.isLoading,
        isRefreshing: preciseGraphStatusQuery.isFetching,
        isReadinessLoading: preciseGraphReadinessQuery.isLoading,
        isTriggeringAnalysis: requestPreciseGraphAnalysisMutation.isPending,
        canRunAnalysis: canRunPreciseGraphAnalysis,
        readinessError: preciseGraphReadinessQuery.error instanceof Error
          ? preciseGraphReadinessQuery.error.message
          : null,
        onOpenGraph: () => navigate(`/novels/${id}/graph`),
        onOpenModelSettings: () => navigate("/settings"),
        onRunAnalysis: () => requestPreciseGraphAnalysisMutation.mutate(),
      }}
