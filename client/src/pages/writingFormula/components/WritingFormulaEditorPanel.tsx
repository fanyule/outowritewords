...
const EDITOR_TEXT_INPUT_CLASS =
  "w-full rounded-xl border border-border/70 bg-background/70 px-3 py-2.5 text-sm text-foreground shadow-sm outline-none transition-colors placeholder:text-muted-foreground/70 focus-visible:border-primary/50 focus-visible:ring-2 focus-visible:ring-primary/20";

const EDITOR_TEXTAREA_CLASS = `${EDITOR_TEXT_INPUT_CLASS} p-3`;
const EDITOR_SECTION_CLASS = "app-shell-panel-soft space-y-4 p-5";
const EDITOR_NOTICE_CLASS = "app-state-panel-muted px-4 py-4 text-sm leading-7 text-muted-foreground";
const EDITOR_META_PILL_CLASS =
  "rounded-lg border border-border/70 bg-background/70 px-2.5 py-1 text-[11px] font-medium text-muted-foreground";
const EDITOR_MUTED_CHIP_CLASS =
  "rounded-full border border-border/70 bg-background/70 px-2 py-0.5 text-[11px] text-muted-foreground";
const EDITOR_LIST_CARD_CLASS = "rounded-xl border border-border/70 bg-background/70 px-3 py-3";
const EDITOR_LIST_EMPTY_CLASS =
  "app-state-panel-muted px-3 py-3 text-sm leading-6 text-muted-foreground";
...
const FEATURE_DECISION_META: Record<NonNullable<StyleProfileFeature["selectedDecision"]>, { label: string; className: string }> = {
  ...
  remove: {
    label: "剥离",
    className: "border border-rose-500/25 bg-rose-500/10 text-rose-600",
  },
};
    ...
    <div className="app-shell-panel-soft space-y-2 p-4">
      ...
      {entries.length > 0 ? (
        <div className="grid gap-2">
          {entries.map((entry) => (
            <div key={`${props.section}-${entry.key}`} className={EDITOR_LIST_CARD_CLASS}>
        ...
        <div className={EDITOR_LIST_EMPTY_CLASS}>
      ...
      <details className="rounded-xl border border-border/70 bg-background/70">
        ...
        <div className="space-y-3 border-t border-border/70 px-3 py-3">
          ...
          {parseError ? (
            <div className="app-warning-panel rounded-xl border px-3 py-2 text-xs leading-6">
              当前 JSON 结构无法正常识别。保存时系统会尽量回退为空对象，建议先修正格式再保存。
            </div>
          ) : null}
          <textarea
            className={`${EDITOR_TEXTAREA_CLASS} min-h-[190px] font-mono text-xs`}
            value={props.value}
...
export default function WritingFormulaEditorPanel(props: WritingFormulaEditorPanelProps) {
  ...
  return (
    <Card data-writing-formula-editor-panel tabIndex={-1} className="app-modal-surface border-border/70">
      ...
      <CardContent className="space-y-5 pt-5">
        {!selectedProfile ? (
          ...
          <>
            ...
            <div className={EDITOR_NOTICE_CLASS}>
            ...
            <div className={EDITOR_SECTION_CLASS}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <FieldBlock label="写法名称" hint="这是你以后在列表里识别这套写法的主标题，尽量写出题材或读感。">
                  <input
                    ...
                    className={EDITOR_TEXT_INPUT_CLASS}
                    value={editor.name}
              ...
              >
                <textarea
                  className={`${EDITOR_TEXTAREA_CLASS} min-h-[96px]`}
            ...
            || editor.sourceContent.trim() ? (
              <div className={EDITOR_SECTION_CLASS}>
                ...
                >
                  <textarea
                    className={`${EDITOR_TEXTAREA_CLASS} min-h-[160px]`}
                ...
                <div className="app-shell-panel-soft p-3">
                  ...
                  {editor.extractedFeatures.length > 0 ? (
                    <div className="space-y-3">
                      ...
                      {editor.extractedFeatures.map((feature) => (
                        <label key={feature.id} className="flex items-start gap-2 rounded-xl border border-border/70 bg-background/55 p-3 text-sm">
                          <input
                            type="checkbox"
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                              <span className={EDITOR_META_PILL_CLASS}>
                            ...
                            <span className="mt-2 flex flex-wrap gap-2">
                              {listRulePatchSections(feature.keepRulePatch).length > 0 ? (
                                listRulePatchSections(feature.keepRulePatch).map((label) => (
                                  <span key={`${feature.id}-${label}`} className={EDITOR_MUTED_CHIP_CLASS}>
                                    {label}规则
                                  </span>
                                ))
                              ) : (
                                <span className="rounded-full border border-dashed border-border/70 px-2 py-0.5 text-[11px] text-muted-foreground">
                                  当前只有摘要级规则
                                </span>
                              )}
                            </span>
                          ...
                          <div className="mt-3 grid gap-3 lg:grid-cols-3">
                            {extractionPresets.map((preset) => {
                              ...
                              return (
                                <div
                                  key={preset.key}
                                  className={`rounded-xl border border-border/70 bg-background/70 p-3 ${isSelected ? "border-primary ring-1 ring-primary/20" : ""}`}
                                >
                                  <div className="flex items-center justify-between gap-2">
                                  ...
                                  <div className="mt-3 flex flex-wrap gap-2">
                                    ...
                                    <span className="rounded-full border border-amber-500/25 bg-amber-500/10 px-2 py-0.5 text-[11px] text-amber-600">
                                      弱化 {counts.weaken}
                                    </span>
                                    <span className="rounded-full border border-rose-500/25 bg-rose-500/10 px-2 py-0.5 text-[11px] text-rose-600">
                                      剥离 {counts.remove}
                                    </span>
                                  </div>
                                </div>
                    ...
                    <div className="app-warning-panel rounded-xl border px-3 py-2 text-sm">
            ...
            <div className={EDITOR_SECTION_CLASS}>
              ...
              <textarea
                className={`${EDITOR_TEXTAREA_CLASS} min-h-[110px]`}
            ...
            <div className={EDITOR_SECTION_CLASS}>
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {antiAiRules.map((rule) => (
                  <label key={rule.id} className="flex items-start gap-2 rounded-xl border border-border/70 bg-background/55 p-3 text-sm">
                    <input
                      type="checkbox"
            ...
            <div className="app-state-panel-muted flex flex-wrap items-center justify-between gap-3 px-4 py-3">
              <div className="text-sm leading-6 text-muted-foreground">
