...
function createIdempotencyKey(taskId: string, actionCode: string): string {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6">
      <div className="task-status-summary-grid grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card className="app-shell-panel-soft">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">运行中</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-[1.7rem] font-semibold leading-none tracking-tight">{runningCount}</div>
          </CardContent>
        </Card>
        <Card className="app-shell-panel-soft">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-[1.7rem] font-semibold leading-none tracking-tight">{queuedCount}</div>
          </CardContent>
        </Card>
        <Card className="app-shell-panel-soft">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-[1.7rem] font-semibold leading-none tracking-tight">{failedCount}</div>
          </CardContent>
        </Card>
        <Card className="app-shell-panel-soft">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-[1.7rem] font-semibold leading-none tracking-tight">{completed24hCount}</div>
          </CardContent>
        </Card>
      </div>

      <div className="grid gap-4 xl:grid-cols-[260px_minmax(0,1fr)_360px]">
        <Card className="task-filter-card app-shell-panel">
          ...
          <CardContent className="task-filter-controls grid min-w-0 grid-cols-3 gap-2 xl:grid-cols-1">
            <select
              className="task-filter-kind col-start-1 row-start-1 w-full rounded-xl border border-border/70 bg-card/72 px-3 py-2.5 text-sm text-foreground shadow-sm outline-none transition hover:border-primary/35 focus:border-primary/45 focus:ring-2 focus:ring-primary/20 xl:col-auto xl:row-auto"
              value={kind}
            ...
            <select
              className="task-filter-status col-start-2 row-start-1 w-full rounded-xl border border-border/70 bg-card/72 px-3 py-2.5 text-sm text-foreground shadow-sm outline-none transition hover:border-primary/35 focus:border-primary/45 focus:ring-2 focus:ring-primary/20 xl:col-auto xl:row-auto"
              value={status}
            ...
            <label className="task-filter-pill col-start-3 row-start-1 flex items-center gap-1.5 rounded-xl border border-border/70 bg-card/66 px-2 py-2.5 text-xs text-muted-foreground sm:gap-2 sm:px-3 sm:text-sm xl:col-auto xl:row-auto">
              <input
                type="checkbox"
                checked={onlyAnomaly}
            ...
            />
            <select
              className="task-filter-sort col-start-3 row-start-2 w-full rounded-xl border border-border/70 bg-card/72 px-3 py-2.5 text-sm text-foreground shadow-sm outline-none transition hover:border-primary/35 focus:border-primary/45 focus:ring-2 focus:ring-primary/20 xl:col-auto xl:row-auto"
              value={sortMode}
        ...
        <Card className="app-shell-panel">
          ...
          <CardContent className="space-y-3">
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-xl border p-3 text-left transition-colors ${
                    isSelected
                      ? "border-primary/45 bg-primary/10 shadow-[var(--app-card-shadow)]"
                      : "border-border/60 bg-card/58 hover:border-border/85 hover:bg-card/76"
                  }`}
                  onClick={() => {
                    setSearchParams((prev) => {
                      const next = new URLSearchParams(prev);
                ...
                <div className="mt-2 app-caption-copy">
                  {formatKind(task.kind)} | 进度 {Math.round(task.progress * 100)}%
                </div>
                <div className="mt-1 app-caption-copy">
            ...
            {visibleRows.length === 0 ? (
              <div className="app-state-panel-muted p-6 text-sm text-muted-foreground">
                当前没有符合条件的任务。
              </div>
            ) : null}
          </CardContent>
            ...
            {selectedTask ? (
              <>
                <div className="space-y-1">
                  <div className="font-semibold leading-snug">{selectedTask.title}</div>
                ...
                {selectedTask.kind === "novel_workflow" && selectedTask.checkpointSummary ? (
                  <div className="app-state-panel-muted p-3 text-sm text-muted-foreground">
                ...
                <div className="space-y-2">
                  ...
                  {selectedTaskSteps.length === 0 ? (
                    <div className="app-state-panel-muted px-3 py-2 text-sm text-muted-foreground">暂无步骤状态。</div>
                ...
                {selectedTask.kind === "novel_workflow" && Array.isArray(selectedTaskMeta.milestones) && selectedTaskMeta.milestones.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    {(selectedTaskMeta.milestones as NovelWorkflowMilestone[]).map((item) => (
                      <div
                        key={`${item.checkpointType}:${item.createdAt}`}
                        className="app-shell-panel-soft rounded-xl p-3 text-sm text-muted-foreground"
                      >
              ...
              <div className="app-state-panel-muted p-6 text-sm text-muted-foreground">请选择任务查看详情。</div>
            )}
          </CardContent>
        </Card>
      </div>
