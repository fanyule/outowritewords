...
自动导演继续自动执行更稳了。章节资源准备完成后，点击继续会进入章节写作链路；如果还需要补齐任务单或场景拆解，系统会先补齐资源再继续执行。

- 自动导演不容易停在 92% 反复等待，章节准备完成后会继续推进到正文生成。
- 对缺少执行资源的章节仍会先补齐规划细节，避免跳过必要的章节任务单和场景拆解。
...
如果你已经把云端认证中心仓库放在当前项目的同级目录：

- 主项目：`F:\workspaceAIxiaoshuo\AI-Novel-Writing-Assistant-main`
- 认证中心：`F:\workspaceAIxiaoshuo\auth-center`
...
- 前端：`http://localhost:5173`
- 后端：`http://localhost:3000`
- 认证中心：`http://localhost:5681`

如果你要一起启动桌面壳联调，用：

```bash
pnpm dev:full-local:desktop
```

如果你的 Windows 环境里全局 `pnpm` shim 有冲突，导致 `pnpm dev:full-local` 还没进入仓库脚本就报 `corepack/dist/pnpm.js` 缺失，可以改用下面任一方式绕过全局 shim：
...
本地套件启动器现在还会在启动前检查 `3000 / 5173 / 5681` 这些关键端口。
如果你之前留有旧的 Node / Vite / auth-center 进程没关，它会直接拒绝启动并提示你先清理旧进程，避免出现“浏览器打开的是旧页面、后端打到的是旧接口”的假象。

补充说明：

- `graph-every-novel-main` 不会被单独起成一个 Web 服务，它是由主项目后端按需调用的本地 Python 图谱引擎
- 如果你的 `auth-center` 不在同级目录，可以先设置环境变量 `AI_NOVEL_AUTH_CENTER_ROOT`
- 如果只想启动主项目，不拉起认证中心，可以执行：

```bash
node scripts/dev-local-suite.cjs --skip-auth
```
...
本项目基于https://github.com/ExplosiveCoderflome/AI-Novel-Writing-Assistant开源上面补充
