...
} from "../services/settings/GraphEngineRuntimeSettingsService";
import {
...
const graphEngineRuntimeSettingsSchema = z.object({
  enabled: z.boolean(),
  autoAnalyze: z.boolean(),
  engineRoot: z.string().trim().min(1),
  projectsRoot: z.string().trim().min(1),
  pythonExecutable: z.string().trim().min(1),
  pythonArgs: z.string().trim().max(500).optional().default(""),
  provider: z.string().trim().max(120).optional().default(""),
  baseUrl: z.union([z.string().trim().url("图谱模型 API 地址格式不正确。"), z.literal("")]).optional().default(""),
  apiKey: z.string().optional(),
  clearApiKey: z.boolean().optional(),
  model: z.string().trim().max(200).optional().default(""),
  temperature: z.coerce.number().min(0).max(2),
  maxTokens: z.coerce.number().int().min(512).max(128000),
});

const graphEngineImportProviderSchema = z.object({
  provider: z.string().trim().min(1).max(120),
});
...
function supportsGraphEngineBootstrap(provider: string): boolean {
...
async function buildCustomProviderStatus(item: {
  ...
  return {
    ...
    apiKeyConfigured: Boolean(normalizeOptionalText(item.key)),
...
router.get("/graph-engine-runtime", async (_req, res, next) => {
  try {
    const data = await getGraphEngineRuntimeSettings();
    res.status(200).json({
      success: true,
      data,
      message: "图谱引擎运行设置读取成功。",
...
router.get("/graph-engine-runtime/readiness", async (_req, res, next) => {
  try {
    const data = await novelGraphPreciseAnalysisService.getRuntimeReadiness("__settings__");
    res.status(200).json({
      success: true,
      data,
      message: "图谱引擎运行诊断读取成功。",
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post(
  "/graph-engine-runtime/import-provider",
  validate({ body: graphEngineImportProviderSchema }),
  async (req, res, next) => {
    try {
      const { provider } = req.body as z.infer<typeof graphEngineImportProviderSchema>;
      if (!supportsGraphEngineBootstrap(provider)) {
        throw new AppError("当前厂商暂不适合直接复用到图谱引擎，请手动填写 OpenAI 兼容配置。", 400);
      }

      const secret = await secretStore.getProvider(provider);
      if (!secret && !isBuiltInProvider(provider)) {
        throw new AppError("没有找到这个厂商的已保存配置。", 404);
      }

      const baseUrl = normalizeOptionalText(secret?.baseURL)
        ?? (isBuiltInProvider(provider)
          ? (getProviderEnvBaseUrl(provider) ?? PROVIDERS[provider].baseURL)
          : undefined);
      const model = normalizeOptionalText(secret?.model)
        ?? (isBuiltInProvider(provider)
          ? (getProviderEnvModel(provider) ?? PROVIDERS[provider].defaultModel)
          : undefined);
      const apiKey = normalizeOptionalText(secret?.key)
        ?? (isBuiltInProvider(provider) ? getProviderEnvApiKey(provider) : undefined);

      if (!baseUrl) {
        throw new AppError("当前厂商还没有可复用的 API 地址。", 400);
      }
      if (!model) {
        throw new AppError("当前厂商还没有可复用的模型名称。", 400);
      }
      if (!apiKey) {
        throw new AppError("当前厂商还没有可复用的 API Key。", 400);
      }

      const current = await getGraphEngineRuntimeSettings();
      const imported = await saveGraphEngineRuntimeSettings({
        enabled: current.enabled,
        autoAnalyze: current.autoAnalyze,
        engineRoot: current.engineRoot,
        projectsRoot: current.projectsRoot,
        pythonExecutable: current.pythonExecutable,
        pythonArgs: current.pythonArgs,
      ...
      const providerLabel = normalizeOptionalText(secret?.displayName)
        ?? (isBuiltInProvider(provider) ? PROVIDERS[provider].name : provider);
      ...
      res.status(200).json({
        ...
        data: {
          ...
          importedProviderLabel: providerLabel,
          importedModel: model,
          importedBaseUrl: baseUrl,
        },
        message: `已从 ${providerLabel} 导入图谱模型配置。`,
      } satisfies ApiResponse<{
        settings: typeof imported;
        importedProvider: string;
        importedProviderLabel: string;
...
router.put(
  ...
  validate({ body: graphEngineRuntimeSettingsSchema }),
  async (req, res, next) => {
    try {
      const data = await saveGraphEngineRuntimeSettings(
        req.body as z.infer<typeof graphEngineRuntimeSettingsSchema>,
      );
      res.status(200).json({
        success: true,
        data,
        message: "图谱引擎运行设置保存成功。",
      } satisfies ApiResponse<typeof data>);
    } catch (error) {
      next(error);
    }
