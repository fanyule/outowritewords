...
export class NovelExportService {
  ...
  private novelServiceInstance: NovelService | null = null;

  private getNovelService(): NovelService {
    if (!this.novelServiceInstance) {
      const { NovelService } = require("./NovelService") as typeof import("./NovelService");
      this.novelServiceInstance = new NovelService();
    }
    return this.novelServiceInstance;
  }
  ...
  private async buildExportBundle(novelId: string): Promise<NovelExportBundle> {
    const novelService = this.getNovelService();
    const rawNovel = await novelService.getNovelById(novelId);
    ...
    const [
      ...
      novelService.getWorldSlice(novelId),
      novelService.listCharacterRelations(novelId),
      novelService.listCharacterCastOptions(novelId),
      novelService.getVolumes(novelId),
      novelService.getLatestStateSnapshot(novelId),
      novelService.getPayoffLedger(novelId).catch(() => null),
      novelService.getQualityReport(novelId),
